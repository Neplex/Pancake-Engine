...
namespace PancakeEngine {
    ...
    /**
     * \class Button
     * \brief Represent a button with its different states (pressed, held, released).
     * \details Buttons are created by the engine user via the InputManager class.
     * When one of the keys associated with the button is pressed or released,
     ...
     * The engine user is not supposed to use buttons.
     * \sa Input InputManager
     * @ingroup Inputs
     */
    class Button {
    public:
    ...
    private:
        friend class InputManager; ///< The only class that should update the state of the button is InputManager

        /**
         * \brief Construct a button with the given name and associated to the given keys.
         ...
         * Should be called after all objects no more need to get button state.
         */
        void update();
        ...
        /**
         * \brief Change the state of the button as if it was just released (or just up).
         */
        void release();
