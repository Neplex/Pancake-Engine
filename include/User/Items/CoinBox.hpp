  ...
  void update() override {
    if (coin == 0)
      gameObject->getComponent<PancakeEngine::SpriteRenderer>()->setSprite(
          PancakeEngine::AssetsManager::getSpriteSheet("tiles"), 2, 0);
  }
...
class CoinBox : public PancakeEngine::GameObject {
 public:
  virtual ~CoinBox() = default;

  CoinBox() {
    auto &bc = addComponent<PancakeEngine::BoxCollider>();
    bc.width = bc.height = 70;

    auto &bottom = addComponent<PancakeEngine::BoxCollider>();
    bottom.width = 70;
    bottom.height = 2;
    bottom.offset = sf::Vector2f(0, 35);
    bottom.isTrigger = true;
    addComponent<PancakeEngine::SpriteRenderer>().setSprite(PancakeEngine::AssetsManager::getSpriteSheet("tiles"), 1,
                                                            0);
