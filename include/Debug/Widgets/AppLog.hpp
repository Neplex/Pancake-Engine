...
/**
 * @struct AppLog
 * @brief It represents a logger used in debug. It uses the ImGui library and is based on the examples of @file
 * imgui_demo.cpp .
 * @details Use the draw function in between ImGui update and render to draw
 * the logger. Add a log with the AddLog function.
 * @example
 * @code
...
struct AppLog {
   ...
   * @details Should be called between ImGui update and render to draw the
   * logger.
   * @param title The title of the logger
   * @param p_open A pointer to the boolean that indicates if the logger should
   * be drawn or not.
   */
  void Draw(const char *title, bool *p_open = nullptr) {
    ...
    if (Filter.IsActive()) {
      const auto *buf_begin = Buf.begin();
