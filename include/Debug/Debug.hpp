...
class Debug {
 public:
  ...
  static void log(std::string loggerName, std::string message) {
    if (initialized) mainDebugMenu.logLogger(loggerName, message + "\n");
  }
  ...
  static void addLogger(std::string name) {
    if (initialized) mainDebugMenu.addLogger(name);
  }
 ...
 private:
  friend class Engine;  ///< The engine will be the one to init, update, and
                        ///< shut down the debug system.
  ...
  static MainDebugMenu mainDebugMenu;          ///< the main debug menu of the debug,
                                               ///< will be drawn on the given window.
   ...
   * @attention Debug does not draw itself the gui. It's #window that should
   * call the render method of Debug. But if another window that #window is
   * used to init calls render, don't know what will arrive...
   * @todo resolve this problem with rendering, the debug should draw itself on
   * the window.
   * @see Window
   */
