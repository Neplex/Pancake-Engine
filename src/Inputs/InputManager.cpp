      ...
      case sf::Event::JoystickMoved: {
        if (event.joystickMove.axis == sf::Joystick::X || event.joystickMove.axis == sf::Joystick::PovX) {
          if (event.joystickMove.position > 50) {
            ...
            if (XaxisToButton.count(pad)) XaxisToButton[pad]->press();
            ...
            std::pair<int, int> pad(event.joystickButton.joystickId, 1);
            if (XaxisToButton.count(pad)) XaxisToButton[pad]->release();
            std::pair<int, int> pad2(event.joystickButton.joystickId, -1);
            if (XaxisToButton.count(pad2)) XaxisToButton[pad2]->release();
          }
        }
        ...
        if (event.joystickMove.axis == sf::Joystick::Y || event.joystickMove.axis == sf::Joystick::PovY) {
          if (event.joystickMove.position > 50) {
            std::pair<int, int> pad(event.joystickButton.joystickId, 1);
            if (YaxisToButton.count(pad)) YaxisToButton[pad]->press();
            ...
            std::pair<int, int> pad(event.joystickButton.joystickId, 1);
            if (YaxisToButton.count(pad)) YaxisToButton[pad]->release();
            std::pair<int, int> pad2(event.joystickButton.joystickId, -1);
            if (YaxisToButton.count(pad2)) YaxisToButton[pad2]->release();
          }
        }
      }
...
void InputManager::createButton(const std::string &name, int joystickNumber, int axisValue, int axis) {
  ...
  auto *b = new Button(name, axisValue, joystickNumber);
  buttons.push_back(b);
  Input::buttons[b->name] = b;
  const std::pair<int, int> pad(joystickNumber, axisValue);
...
void InputManager::createButton(const std::string &name, int joystickNumber, int joystickButton,
                                const std::function<void()> &callback) {
  for (const auto &button : buttons) {
    assert(name != button->name);
  }

  auto *b = new Button(name, joystickButton, joystickNumber);
  b->pressedCallback = callback;
  buttons.push_back(b);
  Input::buttons[b->name] = b;

  const std::pair<int, int> pad(joystickNumber, joystickButton);
  joystickToButton[pad] = b;
}

void InputManager::createButton(const std::string &name, const std::vector<sf::Keyboard::Key> &keys,
                                const std::function<void()> &callback) {
  for (const auto &button : buttons) {
    assert(name != button->name);
  }

  auto *b = new Button(name, keys);
  b->pressedCallback = callback;
  buttons.push_back(b);
  Input::buttons[b->name] = b;

  for (auto key : keys) {
    keyToButtons[key].push_back(b);
  }
}
