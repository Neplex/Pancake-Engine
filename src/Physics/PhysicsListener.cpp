...
void PhysicsListener::onContact(b2Contact *contact,
  ...
  if (contact->IsTouching() || exiting) {
    // If only one of them is a sensor call OnTrigger on the sensor
    // GameObject
    if (contact->GetFixtureA()->IsSensor() ^ contact->GetFixtureB()->IsSensor()) {
      ...
      const Collision collA(*static_cast<Collider *>(contact->GetFixtureA()->GetUserData()),
                            *static_cast<Collider *>(contact->GetFixtureB()->GetUserData()), contact->GetRestitution(),
                            contact->GetFriction(), contact->GetTangentSpeed());
      ...
      const Collision collB(*static_cast<Collider *>(contact->GetFixtureB()->GetUserData()),
                            *static_cast<Collider *>(contact->GetFixtureA()->GetUserData()), contact->GetRestitution(),
                            contact->GetFriction(), contact->GetTangentSpeed());
