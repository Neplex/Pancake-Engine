...
TileMapRenderer::TileMapRenderer() { setTileMap(AssetsManager::getDefaultTileMap()); }

TileMapRenderer::~TileMapRenderer() { clearGrid(); }
...
void TileMapRenderer::setTileMap(const TileMap &map) {
  // Remove old map
  clearGrid();

  // Init (build the matrix of chunks)
  chunk_width = static_cast<unsigned>(floor(CHUNK_SIZE / map.tile_width)) * map.tile_width;
  chunk_height = static_cast<unsigned>(floor(CHUNK_SIZE / map.tile_height)) * map.tile_height;
  ...
  for (TileMap::Tile t : map.map) {
    ...
    s.setPosition(t.x * map.tile_width - (chunk_x * chunk_width) + map.tile_width / 2,
...
std::vector<sf::Sprite> TileMapRenderer::getChunksSprites() const {
  std::vector<sf::Sprite> sprites;
  for (int i = 0; i < nb_column; ++i) {
    for (int j = 0; j < nb_row; ++j) {
      sf::Sprite sprite(textureGrid[i][j]->getTexture());
      // Set position corresponding to the matrix position
      sprite.setPosition(i * chunk_width - (nb_column / 2.0 * chunk_width) + marge_x / 2,
